...
# service-discover owns its zimbraServiceEnabled value: `service-discover setup`
# treats the absence of it, cluster-wide, as "I am the first instance" and only
# then creates the mesh credentials in LDAP. Bootstrap must therefore preserve an
# existing value but never introduce one, or setup takes the join path and fails
# downloading credentials that do not exist yet (CO-4122).
sub isServiceDiscoverEnabledInLdap {
    my $enabled = 0;

    open( ZMPROV, "$ZMPROV gs $config{HOSTNAME} zimbraServiceEnabled 2>/dev/null|" );
    while (<ZMPROV>) {
        $enabled = 1 if (/^zimbraServiceEnabled:\s+service-discover\s*$/);
    }
    close(ZMPROV);
...
sub configSetEnabledServices {
    ...
    foreach my $p ( keys %installedPackages ) {
        if ( $p eq "carbonio-core" ) {
            push( @installedServiceList, ( 'zimbraServiceInstalled', 'stats', 'zimbraServiceInstalled', 'service-discover' ) );
            next;
        }
        $p =~ s/carbonio-//;
    ...
    if ( isServiceDiscoverEnabledInLdap() ) {
        detail("Preserving service-discover in zimbraServiceEnabled.");

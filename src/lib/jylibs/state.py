...
class State:
	...
	# Systemd targets - if any is enabled, we're in a systemd environment
	systemdTargets = [
	...
	def isSystemd(self):
		"""Check if systemd is available by testing if any target is enabled"""
		for target in State.systemdTargets:
			if self.isSystemdTargetEnabled(target):
				return True
		return False

	def isSystemdTargetEnabled(self, target):
		"""Check if a systemd target is enabled"""
		try:
			import subprocess
			result = subprocess.call(["systemctl", "is-enabled", target],
	...
	def controlProcess(self, process, action_value):
		...
		# When systemd is detected (at least one target enabled), skip restart/reload
		# actions. Systemd path units will handle config changes automatically.
		if self.isSystemd() and action != "status":
			return 0

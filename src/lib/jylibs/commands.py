...
exe = {
	...
	'DIRECTORY-SERVER' : "(systemd-target)",
	'SERVICE-DISCOVER' : "(systemd-service)",
	}
...
def systemd_managed_service(sArgs=None, rArgs=None):
	"""No-op for services managed by systemd (directory-server, service-discover).
	These services are controlled via systemd targets/units, not via traditional
	zimbra control scripts. This function allows the watchdog to track them
	without errors."""
	Log.logMsg(5, "systemd_managed_service: sArgs='%s', rArgs='%s' (no-op)" % (sArgs, rArgs))
	return (0, "running", "OK")
...
commands = {
	...
	"directory-server" : Command(
		desc = "directory-server (systemd target)",
		name = "directory-server",
		func = systemd_managed_service,
	),
	"service-discover" : Command(
		desc = "service-discover (systemd service)",
		name = "service-discover",
		func = systemd_managed_service,
	),
	}

miscCommands = ["garpu","garpb","gamau"]
